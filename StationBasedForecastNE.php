...
<script>
        ...
        window.onload = function () {
            ...
            $.get("textFile/stationPosition.txt", function (data) {
            ...
            $.get("textFile/tooltip.txt", function (data2) {
...
<script>
        ...
        $('#sht').delegate("a", "click", function () {
            ...
            $(".positionMarker").click(function () {
                ...
                var dangerLevel = [];
                var HighestLevel = [];
                $.get("DataBaseNE/" + btn + ".csv", function (data) {
                    var allLines = data.split('\n');

                    if (allLines.length > 0) {
                        for (var i = 1; i <= allLines.length - 5; i++) {
                            var dataPoint = allLines[i].split(',');
                            dataPoints.push({ x: Date.parse(dataPoint[0]), y: parseFloat(dataPoint[1]) });
                        };
                        for (var i = allLines.length - 5; i < allLines.length; i++) {
                            var dataPoint = allLines[i].split(',');
                        ...
                        for (var i = allLines.length - 1; i < allLines.length; i++) {
                            var dataPoint = allLines[i].split(',');
